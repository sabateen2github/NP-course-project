...
public class CustomerAuth {
    /**
     * This method is is the main entry point for the login process, as a customer.
     *
     * @param scanner
     * @return
     * @throws IOException
    ...
    public static Session handleLogin(Scanner scanner) throws IOException, ClassNotFoundException {
        ...
        while (true) {
            ...
            try (Socket server = new Socket(SERVICE_HOST, SERVICE_PORT)) {
                ...
                objectOutputStream.writeInt(OpCodes.OP_LOGIN_CUSTOMER);
    ...
    /**
     * This method is the main entry point for the signup process, as a customer.
     *
     * @param scanner
     * @return
     * @throws IOException
    ...
    public static long handleSignup(Scanner scanner) throws IOException, WrongPrivateKeyException {
        while (true) {
            try (Socket server = new Socket(SERVICE_HOST, SERVICE_PORT)) {
                ...
                if (signupLoop(username, password, objectInputStream, objectOutputStream, scanner)) {
                    long id = objectInputStream.readLong();
                    System.out.println("Customer has been created. #ID: " + id);
                    return id;
    ...
    /**
     * This method communicates with the server in order to signup a client
     *
     * @param username
     * @param password
     ...
     * @throws IOException
     */
    private static boolean signupLoop(String username, String password, ObjectInputStream objectInputStream, ObjectOutputStream objectOutputStream, Scanner scanner) throws IOException {
        objectOutputStream.writeInt(OpCodes.OP_SIGN_UP_CUSTOMER);
        objectOutputStream.writeObject(username);
        objectOutputStream.writeObject(password);
        objectOutputStream.flush();
